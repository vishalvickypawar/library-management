...
<body class="w3-light-grey">
    <div class="w3-content w3-white w3-card-4" style="min-height:1000px">
        ...
        <div class="w3-row w3-content w3-padding w3-margin-top">
          ...
          <a href="recent-upload.do">
            <div class="w3-half tablink w3-bottombar w3-hover-light-grey w3-padding w3-border-teal">PDFs</div>
          </a>
          <a href="recent-upload-video.do">
            <div class="w3-half tablink w3-bottombar w3-hover-light-grey w3-padding">Videos</div>
          </a>
        </div>
        ...
        <div class="topic w3-content w3-padding-bottom">
        	<div class="w3-card-4 w3-margin">
            	...
            	<h2><i class="fa fa-file-pdf-o"></i> &nbsp;List of PDFs</h2></div>
        		<ul class="w3-ul">
        		<c:forEach items="${sessionScope.files }" var="files">
                  <li class="w3-row w3-hover-light-grey">
                    ...
                    <div class="w3-half w3-padding">
                        ...
                        <span>${files.description }</span><br><br>
                        <span class="w3-small"><i class="fa fa-book"></i>
                        <c:choose>
                        <c:when test="${files.subject=='chemistry' }">Chemistry</c:when>
                        <c:when test="${files.subject=='digital-electronics' }">Digital Electronics</c:when>
                        <c:when test="${files.subject=='java' }">Java</c:when>
                        <c:when test="${files.subject=='microprocessor' }">Microprocessor</c:when>
                        <c:when test="${files.subject=='physics' }">Physics</c:when>
                        <c:when test="${files.subject=='web-designing' }">Web Designing</c:when>
                        <c:when test="${files.subject=='other' }">Other</c:when>
                        </c:choose>
                        </span>
                    </div>
                  	<div class="w3-quarter w3-right" style="min-width:170px">
                        <form action="view-pdf.do">
                        <input type="hidden" value="${files.id}" name="file">
						...
						<form action="delete-file.do" onsubmit="initial();">
						...
						<input class="w3-btn w3-padding w3-margin-right w3-margin-top w3-medium w3-right w3-teal w3-hover-red" type="submit" value="Delete">
						</form>
                    </div>
                  </li>
                </c:forEach>
