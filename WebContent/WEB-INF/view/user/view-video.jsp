...
<body class="w3-light-grey">
    <div class="w3-content w3-white w3-card-4" style="min-height:1000px">
        ...
        <div class="w3-card-4 w3-padding-bottom w3-margin-bottom w3-margin-top" style="min-height:855px">
        <div class="w3-container w3-teal w3-center">
          <c:choose>
        <c:when test="${viewvideo.subject=='chemistry' && recent=='no' }">
          <a href="subjects-chemistry-video.do" class="w3-left w3-padding-top"><img src="${back }" width="50px"></a>
        </c:when>
        <c:when test="${viewvideo.subject=='digital-electronics' && recent=='no' }">
          <a href="subjects-digital-electronics-video.do" class="w3-left w3-padding-top"><img src="${back }" width="50px"></a>
        </c:when>
        <c:when test="${viewvideo.subject=='java' && recent=='no' }">
          <a href="subjects-java-video.do" class="w3-left w3-padding-top"><img src="${back }" width="50px"></a>
        </c:when>
        <c:when test="${viewvideo.subject=='microprocessor' && recent=='no' }">
          <a href="subjects-microprocessor-video.do" class="w3-left w3-padding-top"><img src="${back }" width="50px"></a>
        </c:when>
        <c:when test="${viewvideo.subject=='other' && recent=='no' }">
          <a href="subjects-other-video.do" class="w3-left w3-padding-top"><img src="${back }" width="50px"></a>
        </c:when>
        <c:when test="${viewvideo.subject=='physics' && recent=='no' }">
          <a href="subjects-physics-video.do" class="w3-left w3-padding-top"><img src="${back }" width="50px"></a>
        </c:when>
        <c:when test="${viewvideo.subject=='web-designing' && recent=='no' }">
          <a href="subjects-web-designing-video.do" class="w3-left w3-padding-top"><img src="${back }" width="50px"></a>
        </c:when>
        <c:when test="${viewvideo.subject=='web-designing' && recent=='no' }">
          <a href="subjects-web-designing-video.do" class="w3-left w3-padding-top"><img src="${back }" width="50px"></a>
        </c:when>
        <c:when test="${recent=='yes' }">
          <a href="recent-upload-video.do" class="w3-left w3-padding-top"><img src="${back }" width="50px"></a>
        </c:when>
        </c:choose>
  			<h2>${viewvideo.title}</h2>
		</div>
		<div class="w3-content" style="max-width:700px;">
		<div class="w3-margin w3-center">
			<video class="w3-card-2" width="100%" controls>
				<source src="${pageContext.request.contextPath }/files/${viewvideo.filename}">
			</video>
        </div>
        </div>
        </div>
